-- Generation signal: adds a nullable song_generation column to songs (NULL =
-- not yet classified, distinct from an explicit 'unclear' GPT read — same
-- convention as lyrical_address, see supabase/pov-signal-migration.sql) and
-- threads it through every RPC that returns song rows, plus the two write
-- RPCs. No CHECK constraint: validated application-side via
-- lib/tagTaxonomy.ts's coerceGeneration.
--
-- DROP FUNCTION signatures below match this migration's PREREQUISITE state
-- (i.e. after pov-signal-migration.sql is already live, which added
-- lyrical_address to every one of these functions) — Postgres requires the
-- DROP to match what's currently deployed before CREATE OR REPLACE can
-- install a new column/parameter list.
--
...
ALTER TABLE public.songs ADD COLUMN IF NOT EXISTS song_generation text;
...
BEGIN
  ...
  SELECT
    ...
    s.lyrical_address, s.song_generation,
...
BEGIN
  ...
  SELECT
    ...
    f.lyrical_address, f.song_generation,
...
RETURNS TABLE (
  ...
  quality_score float, lyrical_address text, song_generation text,
  brief_embedding vector(1536), distance float
)
LANGUAGE plpgsql AS $$
BEGIN
  RETURN QUERY
...
-- DROP matches the 32-param signature already live after pov-signal-migration.sql
-- (...p_brief_embedding text, p_lyrical_address text).
DROP FUNCTION IF EXISTS public.create_song(
  ...
  float8, float8, boolean, text[], text, text, text, text, text
);
...
BEGIN
  INSERT INTO public.songs (
    ...
    music_supervisor_summary, brief_embedding, lyrical_address, song_generation, updated_at
  ) VALUES (
    ...
    p_music_supervisor_summary, p_brief_embedding::vector(1536), p_lyrical_address, p_song_generation, now()
...
-- DROP matches the 14-param signature already live after pov-signal-migration.sql
-- (...p_brief_embedding text, p_lyrical_address text).
DROP FUNCTION IF EXISTS public.update_song(uuid, text, int, text[], text[], text[], text[], text[], text[], text, boolean, text, text, text);
...
-- 9. list_catalog (admin listing — needed by the backfill script the same
-- way lyrical_address needed it: GET /api/admin/songs -> listSongs() calls
-- this RPC, and the backfill script filters on WHERE song_generation IS NULL
-- client-side using whatever this RPC returns.
...
LANGUAGE sql SECURITY DEFINER AS $$
  SELECT id, title, artist, language, energy, popularity_tier,
    ...
    lyrical_address, song_generation, created_at
  FROM public.songs
